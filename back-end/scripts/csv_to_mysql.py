...
def insert_transceiver(cursor, tag_ref, company_id, balance):
    ...
    if count == 0:
        ...
        # Update existing transceiver's balance
        query = "UPDATE Transceiver SET balance = %s WHERE tagRef = %s"
        cursor.execute(query, (balance, tag_ref))
        print(f"Updated tagRef: {tag_ref} with new balance: {balance}")
...
def insert_data(cursor, conn, df):
    try:
        ...
        for _, row in df.iterrows():
            ...
            if checker:
                print(f"Duplicate passage found: {row['timestamp']}, skipping insert.")
                continue
